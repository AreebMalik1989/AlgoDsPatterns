    ...
    public static void main(String[] args) {
        ...
        for(int i=0; i<100; i++) {
            CompletableFuture.supplyAsync(() -> getOrder(), ioBound)    // performed in ioBound thread pool
                    .thenApplyAsync(order -> enrich(order), cpuBound)    // performed in cpuBound thread pool
                    .thenApplyAsync(order -> performPayment(order), ioBound) // performed in ioBound thread pool
