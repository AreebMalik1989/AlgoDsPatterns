...
1.  Prototype interface: declares the cloning methods. In most cases, it’s a
    single clone method.

2.  Concrete Prototype: class implements the cloning method. In addition to
    copying the original object’s data to the clone, this method may also
    handle some edge cases of the cloning process related to cloning linked
    objects, untangling recursive dependencies, etc.

3.  Client: can produce a copy of any object that follows the prototype
    interface.
"""
...
class SomeComponent:
    ...
    `copy.deepcopy` functions. And any class that want to implement custom
    ...
    def __copy__(self):
        ...
        calls `copy.copy` with this object and returned value is returned as
        the new shallow copy.
        """
        new = self.__class__(
    ...
    def __deepcopy__(self, memo={}):
        ...
        What is the use of argument `memo`? Memo is the dictionary that is used
        by the `deepcopy` library to prevent infinite recursive copies in
        instances of circular references. Pass it to all `deepcopy` calls you
        make in the `__deepcopy__` implementation to prevent infinite
        recursions.
        """
        new = self.__class__(
            self.some_int, self.some_list_of_objects, self.some_circular_ref
...
if __name__ == "__main__":
    ...
    # component.
    deep_copied_component.some_list_of_objects.append("yet another object")
    if component.some_list_of_objects[-1] == "yet another object":
        print(
...
id(deep_copied_component.some_circular_ref.parent):         4471114128
id(deep_copied_component.some_circular_ref.parent.some_circular_ref.parent):         4471114128
