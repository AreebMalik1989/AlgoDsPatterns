class Heap:
    ...
    def push(self, data):
        """
        Append data on the back of the heap,
        sift upwards if heap property is violated.

        :param data: data to be added in heap
        :return: None
        """
    ...
    def pop(self):
        ...
        put last element as new root and sift downwards till heap property is satisfied.
    ...
    def replace(self, new_value):
        """
        Remove root and put new element as root & sift down -> No need to sift up

        :param new_value: new value to be used for replacement
        :return: root value
        """
    ...
    def heapify(self, input_list):
        """
        Each leaf is a trivial sub-heap, so we may begin to call heapify on each parent of a leaf.
        Parents of leaf begin at index n/2.
        As we go up the tree making sub-heaps out of unordered elements, we build larger and larger heaps,
        joining them at i'th element with heapify, until the input list is one big heap.

        :param input_list: input list to be heapify
        :return: None
        """
    ...
    def __siftdown(self, index):
        ...
        # Following code works because if the right_child_index is not None, then the left_child_index is als not None
        # This is the property of a complete binary tree,
        # else left will be returned
        best_child_index, best_child_value = (right_child_index, right_child_value) if right_child_index\
            is not None and self.compare(right_child_value, left_child_value) else (left_child_index, left_child_value)
    ...
    def compare(self, value1, value2):
        raise NotImplementedError('Should not use base class heap instead use the class MinHeap or MaxHeap.')
